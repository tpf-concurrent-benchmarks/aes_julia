...
function run_cipher()
    ...
    @time cipher_stack = aes_cipher.new_cipher_stack(env_variables["PLAIN_TEXT"], env_variables["ENCRYPTED_TEXT"], env_variables["DECRYPTED_TEXT"], cipher_key, parse(Int, env_variables["BUFFER_SIZE"]), parse(Int, env_variables["N_THREADS"]), parse(Int, env_variables["TASK_PER_THREAD"]))
